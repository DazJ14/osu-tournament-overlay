...
// Hacemos auto-scroll cuando cambia la cantidad de mensajes
watch(() => chatState.messages.length, async () => {
  await nextTick(); // Esperamos a que Vue dibuje el nuevo mensaje en el DOM
...
const getTeamColor = (team: string) => {
  if (team === 'bot') return 'text-yellow-400'; // Ajustado a Tailwind
  if (team === 'left') return 'text-red-500';
  if (team === 'right') return 'text-blue-500'; // Cambiado a blue, corrígelo si era rojo intencionalmente
  return '';
};
</script>

<template>
  <div id="chat" class="p-5 bg-mantle rounded-xl border border-surface-0 w-[50%] h-full overflow-hidden transition-opacity">
